...
def rectContour(contours):
    # find 4 corner area korbe that will be react
    rectCon = []
    max_area = 0
    for i in contours:
...
def reorder(myPoints):
    ...
    myPoints = myPoints.reshape((4, 2)) # REMOVE EXTRA BRACKET ->biggest rect point
    # print(myPoints)
...
def showAnswers(img,myIndex,grading,ans,questions=5,choices=5):
     ...
     for x in range(0,questions):
         ...
         if grading[x] == 1:
             ...
             myColor = (0,0,255)
             correctAns= ans[x]
             cv2.circle(img, ((correctAns*secW)+secW//2,(x*secH)+secH//2),50, (255,0,0), cv2.FILLED)

         cv2.circle(img, (cX, cY), 50, myColor, cv2.FILLED)
     return img

def showAnswersOld(img, myIndex, grading, ans, questions=5, choices=5):
    secW = int(img.shape[1] / questions)  # section width
    secH = int(img.shape[0] / choices)  # section height
